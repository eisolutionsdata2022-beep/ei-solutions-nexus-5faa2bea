...
$csql->execute([get_safe(base64_decode($_POST['vle_id']))]);
$row=$csql->fetch();

if($row['id']>0 AND ($row['createby']==$userdata['username'] OR $userdata['username']==$row['username'])){
...
if($row['id']>0 AND ($row['createby']==$userdata['username'] OR $userdata['username']==$row['username'])){
   /* $vl = $row["username"];
   echo "<script type='text/javascript'>alert('$vl');</script>"; */
...
if(strtolower($status)=='success'){
$sql = $conn->prepare("UPDATE loginusers SET status='approved', remark='".$message."', vle_id='".$userid."', psa_reg_code='".$re."' WHERE id='".$row['id']."' ");
$sql->execute();    
 echo '<div class="alert alert-success" role="alert">'.$message.'!</div>';       
}else{
 echo '<div class="alert alert-danger" role="alert">'.$message.'!</div>';     
...
if($userdata['usertype']=='mainadmin'){
    $stmt = $conn->prepare("select * from loginusers ORDER BY `id` DESC");
    $stmt->execute();
}
